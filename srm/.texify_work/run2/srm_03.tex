...
\begin{solution}{33}{E}
\begin{align*}
\mathrm{LCA}(\mathrm{I}) &= 8.146 \\
\mathrm{LCA}(\mathrm{II}) &= 8.028 \\
\mathrm{LCA}(\mathrm{III}) &= 7.771
\end{align*}
\end{solution}
...
% SIC: source prints "of" for "or" — "The presence of absence of scaling".
...
\[
F = \dfrac{(TSS - RSS)/1}{RSS/(n-2)} = \dfrac{490{,}000 - 250{,}000}{250{,}000/98} = 94.08.
\]
\end{solution}
...
% SIC: source omits the "kg" unit; the other two predictions in this paragraph print "0.90 kg" and "0.80 kg".
Prediction = 1.25.
...
\begin{solution}{57}{A}
$\mathrm{T1}$ has observations with $Z \leqslant 3$ and $Y = \mathrm{A}$ or $\mathrm{B}$, which are observations 1, 5, and 9. The values are 4.75, 4.53, and 3.89, which average to 4.39.

$\mathrm{T2}$ has observations with $Z \leqslant 3$ and $Y = \mathrm{C}$ or $\mathrm{D}$, which are observations 3, 4, and 6. The values are 4.67, 4.56, and 3.91, which average to 4.38.

$\mathrm{T3}$ has observations with $Z > 3$ and $X = \mathrm{F}$, which are observations 2 and 7. The values are 4.67 and 3.90, which average to 4.29.

$\mathrm{T4}$ has observations with $Z > 3$ and $X = \mathrm{M}$, which is observation 8. The value is 3.90.
\end{solution}
...
% SIC: source omits "or"; should read "less than or equal to".
...
\begin{solution}{69}{C}
% SIC: source omits the full stop after "zero".
...
% SIC: source opens the AIC bracket with "[" and closes it with "]" but the inner "(89.45755" is never closed; the intended reading is "[... + 2(k)(89.45755)]/100". Reproduced as printed on all three lines.
